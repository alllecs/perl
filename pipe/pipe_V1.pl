...
while (<STDIN>) {
	if (/(\d+).*(store|load).*(hit|miss).*(VA=)(.{16}).*(PA=)(.{8}).*/) {
		...
		if ($n == 0 and $buf_addr[$m - 3] - $buf_addr[$m - 2] != 0x20 and $buf_addr[$m - 2] - $buf_addr[$m - 1] != 0x20 and $buf_addr[$m + 1] - $buf_addr[$m] != 0x20) {
                                $n = 1;
                                $in++;
#                               print "instr$instr\n";
               } elsif ($n == 1 and $buf_addr[$m - 3] - $buf_addr[$m] == 0x60 and $buf_addr[$m - 2] - $buf_addr[$m] == 0x40 and $buf_addr[$m - 1] - $buf_addr[$m] == 0x20) {
                                $n = 0;
                                $dec++;
              	}
		if (/(\d+).*(dcache).*(store miss).*(VA=)(.{16}).*/) {
			$instr2 = $1;
		} 
		if (/($instr2).*(scache).*(store miss).*(VA=)(.{16}).*/) {
			$va = $5;
                        $m++;
                        $buf_addr[$m] = hex($va);
			if ($n == 0 and $buf_addr[$m - 3] - $buf_addr[$m - 2] != 0x20 and $buf_addr[$m - 2] - $buf_addr[$m - 1] != 0x20 and $buf_addr[$m + 1] - $buf_addr[$m] != 0x20) {
